...
class CborEncoderTest extends TestCase
{
     ...
     */
    public function testEncodeTimestamp(DateTime $value, string $expectedPrefix): void
    {
    ...
    {
        $dt1 = new DateTime('2024-01-01 00:00:00.000000');
        $dt2 = new DateTime('2024-01-01 12:30:45.500000');

        return [
            'integer-timestamp' => [$dt1, "\xC1"],
            'float-timestamp' => [$dt2, "\xC1"],
        ];
    }

    public function testEncodeTimestampWithMilliseconds(): void
    {
        $dt = new DateTime('2024-01-01 00:00:00.123456');
        $encoded = $this->encoder->encode($dt);

        // Should be tag 1 followed by either int or float
        $this->assertSame("\xC1", $encoded[0]);

        // Verify it contains millisecond precision
        $secondByte = ord($encoded[1]);
        $this->assertTrue($secondByte === 0xFA || $secondByte === 0xFB || $secondByte >= 0x1A);
    }
