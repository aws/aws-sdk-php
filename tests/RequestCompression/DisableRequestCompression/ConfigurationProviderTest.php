...
use Aws\RequestCompression\DisableRequestCompression\ConfigurationProvider;
use Aws\LruArrayCache;
use Aws\RequestCompression\DisableRequestCompression\Configuration;
use Aws\RequestCompression\DisableRequestCompression\ConfigurationInterface;
use Aws\RequestCompression\Exception\ConfigurationException;
use GuzzleHttp\Promise;
use Yoast\PHPUnitPolyfills\TestCases\TestCase;

/**
 * @covers \Aws\RequestCompression\DisableRequestCompression\ConfigurationProvider
 */
...
EOT;
    ...
    {
        self::$originalEnv = [
            'disable_request_compression' => getenv(ConfigurationProvider::ENV_DISABLE_REQUEST_COMPRESSION) ?: '',
            'home' => getenv('HOME') ?: '',
    ...
    {
        putenv(ConfigurationProvider::ENV_DISABLE_REQUEST_COMPRESSION . '=');
    ...
    {
        putenv(ConfigurationProvider::ENV_DISABLE_REQUEST_COMPRESSION . '=' .
            self::$originalEnv['disable_request_compression']);
    ...
    {
        $this->clearEnv();
        putenv(ConfigurationProvider::ENV_DISABLE_REQUEST_COMPRESSION . '=true');
    ...
    public function testGetsHomeDirectoryForWindowsUsers()
    {
        ...
        $ref = new \ReflectionClass(
            'Aws\RequestCompression\DisableRequestCompression\ConfigurationProvider'
        );
    ...
    public function testSelectsEnvironmentOverIniConfiguration()
    {
        $dir = $this->clearEnv();
        $expected = new Configuration(false);
        putenv(ConfigurationProvider::ENV_DISABLE_REQUEST_COMPRESSION . '=false');
        file_put_contents($dir . '/config', $this->iniFile);
        putenv('HOME=' . dirname($dir));
        putenv(ConfigurationProvider::ENV_PROFILE . '=custom');
    ...
    public function testCreatesFromCache()
    {
        ...
        $provider = ConfigurationProvider::defaultProvider([
            'disable_request_compression' => $cache,
            'region' => 'us-east-1'
        ]);
        /** @var ConfigurationInterface $result */
        $result = $provider()->wait();
