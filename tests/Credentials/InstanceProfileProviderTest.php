...
class InstanceProfileProviderTest extends TestCase
{
    ...
    public function testAppliesDefaultEndpoint(){
    ...
    public function testAppliesCustomEndpoint(){
        $client = function (RequestInterface $req) {
            $this->assertSame(
               '[fd00:ec2::254]',
                $req->getUri()->getHost()
    ...
    public function testAppliesDefaultIpVersion(){
        $client = function (RequestInterface $req) {
            $this->assertNotEmpty(
                $req->getHeader('force_ip_resolve')
            );
            $this->assertSame(
                '169.254.169.254',
                $req->getUri()->getHost()
            ...
            $this->assertSame(
                'v4',
                $req->getHeader('force_ip_resolve')[0]
            );
            $expiry = time() + 1000;
            $creds = ['foo_key', 'baz_secret', 'qux_token', "@{$expiry}"];
            return new FulfilledPromise(new Response(
    ...
    public function testAppliesCustomIpVersion(){
        $client = function (RequestInterface $req) {
            $this->assertNotEmpty(
                $req->getHeader('force_ip_resolve')
            );
     ...
     * @expectedExceptionMessage Invalid input for endpoint_mode provided.  Valid inputs include: IPv4, IPv6
     */
    public function testFailsOnBadCustomIpVersion(){
        ...
        $provider = new InstanceProfileProvider([
            'client' => $client,
            'endpoint_mode' => 'IPvInvalid',
            'retries' => 5,
        ]);
        $provider()->wait();
    }

    public function testPrioitizesClientConfigFirst(){
        $fileContents = <<<EOT
[default]
ec2_metadata_service_endpoint_mode = ipv6
ec2_metadata_service_endpoint = config-file-endpoint.com
EOT;
        $tmpFile = sys_get_temp_dir() . '/test.ini';
        file_put_contents($tmpFile, $fileContents);
        putenv("AWS_CONFIG_FILE={$tmpFile}");
        putenv("AWS_EC2_METADATA_SERVICE_ENDPOINT_MODE={IPv4}");
        putenv("AWS_EC2_METADATA_SERVICE_ENDPOINT=endpoint-env.com");
    ...
    public function testPrioitizesEnvironmentSecond(){
        $fileContents = <<<EOT
[default]
ec2_metadata_service_endpoint_mode = ipv6
ec2_metadata_service_endpoint = config-file-endpoint.com
EOT;
        ...
        file_put_contents($tmpFile, $fileContents);
        putenv("AWS_CONFIG_FILE={$tmpFile}");
        putenv("AWS_EC2_METADATA_SERVICE_ENDPOINT_MODE=IPv6");
        putenv("AWS_EC2_METADATA_SERVICE_ENDPOINT=endpoint-env.com");
    ...
    public function testPrioitizesConfigFileThird(){
        $fileContents = <<<EOT
[default]
ec2_metadata_service_endpoint_mode = IPv6
ec2_metadata_service_endpoint = config-file-endpoint.com
