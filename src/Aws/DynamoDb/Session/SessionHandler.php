...
class SessionHandler
{
    ...
    public function createSessionsTable($readCapacityUnits, $writeCapacityUnits)
    {
        ...
        if (version_compare($this->client->getDescription()->getApiVersion(), '2012-08-10', '<')) {
    ...
    public function garbageCollect()
    {
        ...
        $hashKey   = $this->config->get('hash_key');
        $expires   = (string) time();
        $isOldApi  = (bool) version_compare($this->client->getDescription()->getApiVersion(), '2012-08-10', '<');
