...
/**
 * Gets an environment variable in a thread safe way
 *
 * @param $variableName
 * @return string|bool
 */
function get_environment_variable($variableName)
{
    $environmentVariable = getenv($variableName);
    // getenv() is not thread safe - fall back to $_SERVER
    if ($environmentVariable === false) {
        $environmentVariable = isset($_SERVER[$variableName])
 ...
 * @return string|bool
 */
function get_config_variable($variableName, $filePath = '/.aws/config', $profileName = null)
{
    ...
    if (!isset($filePath)) {
        if ($homeDir = getenv('HOME')) {
            ...
            $filePath = ($homeDrive && $homePath) ? $homeDrive . $homePath : null;
        }
        $filePath .= '/.aws/config';
    }
    ...
    $profileData = \Aws\parse_ini_file($filePath, true, INI_SCANNER_RAW);
    return isset($profileData[$profileName][$variableName])
        ? $profileData[$profileName][$variableName]
        : false;
}
