...
class EndpointParameterMiddleware
{
     ...
     * @return \Closure
     */
    public static function wrap(Service $service, $args = [])
    {
        return function (callable $handler) use ($service, $args) {
            return new self($handler, $service, $args);
        };
    }

    public function __construct(callable $nextHandler, Service $service, $args)
    {
    ...
    public function __invoke(CommandInterface $command, RequestInterface $request)
    {
        ...
        if (empty($this->args['disable_host_prefix_injection'])) {
