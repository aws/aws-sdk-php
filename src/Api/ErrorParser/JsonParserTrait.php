...
trait JsonParserTrait
{
    ...
    private function genericHandler(ResponseInterface $response): array
    {
        ...
        if (!$error_code && $response->hasHeader('X-Amzn-Errortype')) {
            $error_code = $this->parseErrorTypeHeader($response);
        }
     ...
     * @return string|null
     */
    private function parseErrorTypeHeader(ResponseInterface $response): ?string
    {
        $error_code = $response->getHeaderLine('X-Amzn-Errortype');
