...
class ConfigurationResolver
{
    ...
    {
        if ($type === 'bool') {
            // INI_SCANNER_TYPED already yields a bool; env() passes a string.
            if (is_bool($value)) {
                return $value;
            }
            if ($value === '') {
            ...
            // boolean_value() returns null when it can't interpret the value;
        ...
        if ($type === 'int') {
            ...
            // Use !== false so a valid "0" is not dropped.
            $int = filter_var($value, FILTER_VALIDATE_INT);
            return $int !== false ? $int : $value;
        }

        // TYPED may return an int/float/bool for a numeric or keyword value;
        // a 'string' key must still resolve to a string, as it did under NORMAL.
        if ($type === 'string') {
